...
5. Compare models using test-set validation.
---
...
```{r}
# Fit best subset selection using all predictors (excluding the intercept and raw wage)
# logwage is the outcome variable; we remove `wage` to avoid redundancy
regfit.full <- regsubsets(logwage ~ . - 1, data = select(Wage, -wage), nvmax = ncol(Wage) - 1)
...
# Plot Mallows’ Cp — an estimate of test error; lower is better
plot(reg.sum$cp, type = "l", xlab = "p", ylab = "Cₚ")
...
regfit.age2 <- regsubsets(logwage ~ . - wage + I(age^2), data = Wage,nvmax = ncol(Wage) - 1)
