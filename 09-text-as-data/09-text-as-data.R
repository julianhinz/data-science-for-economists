...
setwd("~/work/Teaching/data-science-for-economists/09-text-as-data")
...
# Now let’s use the gsub command to remove all \t (\t usually denotes a tab character) while \n (starts on a new line)
gsub("\t", "", "Today class it's very fun\n\n\t\t\t\t\t\t\t")
gsub("\t|\n", "", "Today class it's very fun\n\n\t\t\t\t\t\t\t")

# none of the words in some_text start with the letter "T"
some_text<-c("This","topic","is","so","musch","fun")
...
# we need to “escape” the character using single quotation marks wrapped around a double \\ as follows:
text_chunk<-c("[This topic is not so fun]")
gsub('\\[|\\]',"", text_chunk)
...
# Split sentences into words: \\s+ matches one or more whitespace characters
...
# Your existing code to remove stop words and numbers
...
# Represent in a Wordcloud the main keywords
...
read_my_xml <- function(x) {
  ...
  temp <- data[1,3] %>%
    ...
    filter(word %in% c("services") | grepl("^agric", word)) %>%  # Match words starting with "agricult"
...
table <- table %>%
  ...
  select(-agricultural, -agriculture)  # Optionally drop the original columns
table
...
chi_sq_result <- chisq.test(table$agriculture_combined, table$services, correct = FALSE)

# Print the chi-square result
print(chi_sq_result)

# PRINT DEPTHNESS OF TRADE AGREEMENTS

dat <- map_df(my_files, read_my_xml)

plot_data <- dat %>% 
  mutate(year = as.POSIXct(year, format = "%Y-%m-%d")) %>% 
  mutate(year = format(year, format = "%Y")) %>% 
  select(year, tot_words) %>% 
  filter(tot_words != 0) %>% 
  group_by(year) %>% 
  mutate(tot_words_m = mean(tot_words, na.rm = TRUE)) %>% 
  select(tot_words_m, year) %>% 
  distinct() %>% 
  ungroup() %>% 
  mutate(year = as.numeric(year)) 


plot_data %>% ggplot(aes(x = year, y = tot_words_m)) +
  geom_line(color = "blue") +  # Line color blue
  geom_point(shape = 21, fill = "red", color = "black", size = 3) +  # Point enhancements
  scale_x_continuous(breaks = seq(1945, 2017, 10)) +  # Treat x as continuous
  xlab("Year") + 
  ylab("Average Number of Words") +  # More descriptive y-axis label
  labs(title = "Depth of Trade Agreements Over Time") +  # More descriptive title
  ...
  theme(
    ...
    axis.title.x = element_text(size = 12),  # Increase axis label size
    axis.title.y = element_text(size = 12)  # Increase axis label size
  )

# Would you fo it differently?
